...
function augment(regex, captureNames, xSource, xFlags, isNotNative, isInternalOnly) {
    ...
    if (regex.__proto__) {
        regex.__proto__ = XRegExp$1.prototype;
    } else {
        for (p in XRegExp$1.prototype) {
            // An `XRegExp.prototype.hasOwnProperty(p)` check wouldn't be worth it here, since this
            // is performance sensitive, and enumerable `Object.prototype` or `RegExp.prototype`
            // extensions exist on `regex.prototype` anyway
            regex[p] = XRegExp$1.prototype[p];
        }
    }
...
function prepareOptions(value) {
    ...
    if (isType(value, 'String')) {
        XRegExp$1.forEach(value, /[^\s,]+/, (match) => {
            options[match] = true;
        });
...
function runTokens(pattern, flags, pos, scope, context) {
    ...
    while (i--) {
        ...
        match = XRegExp$1.exec(pattern, t.regex, pos, 'sticky');
        if (match) {
            result = {
...
function prepareJoin(patterns) {
    ...
    for (let i = 0; i < patterns.length; ++i) {
        ...
        if (XRegExp$1.isRegExp(pattern)) {
            numPriorCaptures = numCaptures;
            captureNames = (pattern[REGEX_DATA] && pattern[REGEX_DATA].captureNames) || [];

            // Rewrite backreferences. Passing to XRegExp dies on octals and ensures patterns are
            // independently valid; helps keep this simple. Named captures are put back
            output.push(nativ.replace.call(XRegExp$1(pattern.source).source, parts, rewrite));
        } else {
            output.push(XRegExp$1.escape(pattern));
        }
    }
 ...
 * XRegExp(/regex/);
 */
function XRegExp$1(pattern, flags) {
    if (XRegExp$1.isRegExp(pattern)) {
        if (flags !== undefined) {
    ...
    if (XRegExp$1.isInstalled('astral') && !flags.includes('A')) {
    ...
    if (!patternCache[pattern][flags]) {
        ...
        while (pos < appliedPattern.length) {
            ...
            if (result) {
                ...
                const token = XRegExp$1.exec(appliedPattern, nativeTokens[scope], pos, 'sticky')[0];
                output += token;
                pos += token.length;
...
XRegExp$1.prototype = new RegExp();
...
XRegExp$1._clipDuplicates = clipDuplicates;
XRegExp$1._hasNativeFlag = hasNativeFlag;
XRegExp$1._registeredFlags = function _getRegisteredFlags() {
    return registeredFlags;
};
XRegExp$1._dec = dec;
XRegExp$1._hex = hex;
XRegExp$1._pad4 = pad4;
 ...
 */
XRegExp$1.addToken = (regex, handler, options) => {
    options = options || {};
 ...
 * }
 */
XRegExp$1.cache = (pattern, flags) => {
    if (!regexCache[pattern]) {
        regexCache[pattern] = {};
    }
    return regexCache[pattern][flags] || (
        regexCache[pattern][flags] = XRegExp$1(pattern, flags)
    );
};

// Intentionally undocumented; used in tests
XRegExp$1.cache.flush = (cacheName) => {
 ...
 * // -> 'Escaped\?\ <\.>'
 */
XRegExp$1.escape = (str) => nativ.replace.call(toObject(str), /[-\[\]{}()*+?.,\\^$|#\s]/g, '\\$&');
 ...
 * // result -> ['2', '3', '4']
 */
XRegExp$1.exec = (str, regex, pos, sticky) => {
    let cacheKey = 'g';
    let addY = false;
    let fakeY = false;
    let match;
 ...
 * // evens -> [2, 4]
 */
XRegExp$1.forEach = (str, regex, callback) => {
    let pos = 0;
    let i = -1;
    let match;

    while ((match = XRegExp$1.exec(str, regex, pos))) {
 ...
 */
XRegExp$1.globalize = (regex) => copyRegex(regex, {addG: true});
 ...
 */
XRegExp$1.isInstalled = (feature) => !!(features[feature]);
 ...
 */
XRegExp$1.isRegExp = (value) => toString.call(value) === '[object RegExp]'; // isType(value, 'RegExp');
 ...
 */
XRegExp$1.match = (str, regex, scope) => {
 ...
 */
XRegExp$1.matchChain = (str, chain) => (function recurseChain(values, level) {
    ...
    for (let i = 0; i < values.length; ++i) {
        XRegExp$1.forEach(values[i], item.regex, addMatch);
    }
 ...
 */
XRegExp$1.replace = (str, search, replacement, scope) => {
    const isRegex = XRegExp$1.isRegExp(search);
    ...
    if (isRegex) {
        ...
        s2 = new RegExp(XRegExp$1.escape(String(search)), 'g');
    }
 ...
 * ]);
 */
XRegExp$1.replaceEach = (str, replacements) => {
    let i;
    let r;

    for (i = 0; i < replacements.length; ++i) {
        r = replacements[i];
        str = XRegExp$1.replace(str, r[0], r[1], r[2]);
    }
 ...
 * // -> ['..', 'word', '1', '..']
 */
XRegExp$1.split = (str, separator, limit) => fixed.split.call(toObject(str), separator, limit);
...
XRegExp$1.test = (str, regex, pos, sticky) => !!XRegExp$1.exec(str, regex, pos, sticky);
 ...
 */
XRegExp$1.join = function(patterns, separator, flags) {
    separator = separator || '';
    const separatorStr = XRegExp$1.isRegExp(separator) ? separator.source : XRegExp$1.escape(separator);
    const output = prepareJoin(patterns);
    return XRegExp$1(output.join(separatorStr), flags);
};
 ...
 * // -> /manbearpig/i
 */
XRegExp$1.union = (patterns, flags, options) => {
    options = options || {};
    const conjunction = options.conjunction || 'or';
    const separator = conjunction === 'none' ? '' : /|/;
    return XRegExp$1.join(patterns, separator, flags);
};
...
fixed.split = function(separator, limit) {
    if (!XRegExp$1.isRegExp(separator)) {
    ...
    XRegExp$1.forEach(str, separator, (match) => {
